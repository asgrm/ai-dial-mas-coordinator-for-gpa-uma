    ...
    async def response(
        ...
        async for chunk in chunks:
            ...
            if delta and delta.custom_content:
                ...
                custom_content_dct = custom_content.dict(exclude_none=True)
        ...
        target_choice_state = {_IS_GPA: True, _GPA_MESSAGES: result_custom_content.state}
        choice.set_state(target_choice_state)
    ...
    def __prepare_gpa_messages(self, request: Request, additional_instructions: Optional[str]) -> list[dict[str, Any]]:
        ...
        for i in range(len(request.messages)):
            ...
            if message.role == Role.ASSISTANT:
                if  message.custom_content and message.custom_content.state and message.custom_content.state.get(_IS_GPA):
                    res_messages.append(request.messages[i-1].dict(exclude_none=True))
                    message_copy = deepcopy(message)
                    message_copy.custom_content.state = message.custom_content.state.get(_GPA_MESSAGES)
                    res_messages.append(message_copy.dict(exclude_none=True))
            ...
            res_messages.append(
                {
                    ...
                    "custom_content": last_user_msg.custom_content.dict(exclude_none=True) if last_user_msg.custom_content else None,
                }
            )
        else:
